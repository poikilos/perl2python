...
class Perl2Python():
    ...
    def replaceVarInString(self, line):
        ...
        if False:
            ...
            newStr = (self.replaceVarInString(left)
                      + ' + ' + self.checkSpecialVariable(varName)
                      + ' + ' + self.replaceVarInString(right))
            self.LOG('final:\n' + newStr + '\n')
            return newStr
    ...
    def parseExpression(self, expression):
        ...
        if mo:
            ...
            self.LOG('left: '+left + '\n')
            middle = mo.group(2)
            self.LOG('middle: '+middle + '\n')
            right = mo.group(3)
            self.LOG('right: '+right + '\n')

            middle = self.replaceVarInString('"'+middle+'"')
            self.LOG('middle2 handled: '+left+'~'+middle+'~'+right+'\n')
            right = self.parseExpression(right)
            self.LOG('middle3 handled: '+left+'~'+middle+'~'+right+'\n')
    ...
    def handleIf(self, line):
        mo = re.match(r'}*\s*(if|elsif)\s*\((.*)\)(.*)\s*$', line)
        if mo:
            ...
            m2 = re.match(r'(-e|-d)\s*"(.*)', ifCondition)
            if m2:
    ...
    def handleWhile(self, line):
        ...
        if mo:
            ...
            self.writeIndent()
            self.write("for " + varName + " in " + fileObject
                       + ".readlines():\n")
            self.increamentTab()
            return True

        # looping through STDIN (while loop)
        mo = re.match(r'\s*while\s*(.*)\<STDIN\>(.*)\s*(.*)\s*$', line)
        if mo:
            self.LOG('SDTIN : ')
            self.writeIndent()
        ...
        mo = re.match(r'(.*)\s*while\s*\((.*)\)(.*)\s*$', line)
        if mo:
            self.LOG('Simple While:')
            mb = re.match(
                r'(.*)\s*while\s*\((.*)\s*\<STDIN\>\s*\)(.*)\s*$',
                line
            )
            # ^ stdin
            if mb:
    ...
    def replaceComparisonOperators(self, param1):
        """
        Comparison operators that dont exist in Python are replaced with
        those that do.
        """
    ...
    def evaluateLogical(self, expression):
        ...
        if mo and expression != "":
            ...
            self.LOG("!! Logical [{}] {} [{}]\n".format(left,
                                                        operator,
                                                        right))
            return (self.evaluateLogical(left) + ' ' + operator + ' '
                    + self.evaluateLogical(right))

        expression = self.replaceComparisonOperators(expression)
        m1 = re.match(r'(.*)\s*(=\~)\s*(.*)', expression)
        if m1:
            self.LOG('+++++ ' + expression + '\n')
            self.LOG('++++! ' + m1.group(2) + '\n')
            self.LOG(
                '++++~'
                + self.checkSpecialVariable(m1.group(1).strip('$@'))
                + '\n'
            )
            return (self.checkSpecialVariable(m1.group(1).strip('$@'))
                    + ' =~ "' + m1.group(3) + '"')

        m2 = re.match(r'(.*)\s*(==|!=|<=?|>=?)\s*(.*)', expression)
        if m2:
            self.LOG('~~~~ ' + expression + '\n')
            self.LOG('~~~! ' + m2.group(2) + '\n')
            return (self.parseExpression(m2.group(1)) + ' '
                    + m2.group(2) + ' '
                    + self.parseExpression(m2.group(3)))
    ...
    def handleComments(self, line):
        mo = re.match(r'\s*#', line) or line.strip() == ''
        if mo:
            self.writeIndent()
    ...
    def handleForEach(self, line):
        # self.write(self.tabCounter
        mo = re.match(r'\s*foreach\s*\((.*)\)\s*$', line)
        if mo:
            ...
            # foreach (with ARGV)
            #   (super specific, could do with broadening in scope)
            m2 = re.match(r'\s*foreach\s*\$(.*)\s*\((.*)\)\s*{\s*$',
                          line)
            if m2:
                # foreach $i (0..$#ARGV)
                # becomes
                ...
                self.writeIndent()
                self.write("for " + variableName
                           + " in xrange (len(sys.argv) - 1):\n")
                self.increamentTab()
                return True
    ...
    def handlePrint(self, line):
        ...
        if mo:
            ...
            if target != '':
                self.write(target + '.write("'
                           + self.parseExpression(printInput) + '")\n')
            else:
        ...
        mo = re.match(r'\s*print\s*"(.*)"[\s;]*$', line)
        if mo:
    ...
    def handleVarAssignment(self, line):
        ...
        if mo:
            ...
            self.writeIndent()
            self.write(varName + " = "
                       + self.parseExpression(expression).strip()
                       + "\n")
            return True
        ...
        if mo:
            ...
            if len(expression) > 0 and expression[0] == '~':
                self.write(self.handleVarName(varName) + " = "
                           + self.handleSed(expression[1:]) + "\n")
                return True
    ...
    def DoConversion(self):
        ...
        for line in self.inFile.readlines():
            ...
            # subroutines without paranthesis
            mo = re.match(r'^\s*sub\s+(.*)\s*$', line)
            if mo:
                funcProto = mo.group(1)
            ...
            # push
            mo = re.match(r'^\s*push\s*\(\s*\@(.*)\,\s*(.*)\)\s*;$',
                          line)
            if mo:
                self.writeIndent()
                self.write(mo.group(1) + ".push("
                           + mo.group(2).strip('$') + ")\n")
                continue

            # pop
            mo = re.match(r'^\s*pop\s*\@(.*);$', line)
            if mo:
                self.writeIndent()
            ...
            # unshift
            mo = re.match(r'^\s*unshift\s*\@(.*)\,\s*(.*)\s*;$', line)
            if mo:
                self.writeIndent()
            ...
            if mo:
                ...
                self.writeIndent()
                self.write(assignmentVariable + " = " + string
                           + '.split("' + delineator + '")\n')
                continue

            # join
            mo = re.match(r"(.*)\s*=\s*join\('(.*)'\,\s*(.*)\)\s*;$",
                          line)
            if mo:
                assignmentVariable = mo.group(1)
                string = mo.group(3)
                delineator = mo.group(2)
                self.writeIndent()
                self.write(assignmentVariable + '= ' + delineator
                           + '.join([' + string + '])')
                continue
            ...
            if mo:
                if mo == re.match(r'\@(.*)\s*=\s*(.*);$', line):
                    # ^ arrays are dealt with seperately
                    next
                else:
                    self.writeIndent()
